...
def generate_ai_insight(df_top, frame_stat_summary, person_summary_str, trend_summary_str, frame_trend_str):
    ...
    try:
        ...
        1. [인물 수치 역전 및 급등 최우선 분석]:
           - 전일 대비 인물별 단독 조회수의 역전 현상(예: 당대표 후보 김민석의 급감 대비 최고위원 후보 최민희의 폭발적 급등 및 정청래 추격)을 1순위 핵심 기류로 필수 반영하세요.
           - 투표 전날 당대표 후보군이 정체되고 최고위원 후보가 전면에 부상하는 정치적 신호를 분명히 서술하세요.
        2. [프레임 착시 보정]:
           - '당내/인물' 프레임의 대다수는 전당대회(최고위원/당대표 경선) 관련 콘텐츠입니다. '전당대회/경선' 단일 프레임(5% 안팎)만 보고 전대 관심이 낮다고 판단하지 말고, '당내/인물'과의 결합 비중(20% 이상)을 바탕으로 전대 관심도를 분석하세요.
        3. [자가 단정 및 추측성 서술 절대 금지]:
           - 채널의 '특화 편성', '편성 전략' 등 방송사의 내면 의도를 단정하지 마세요 (시의성 현상일 뿐임).
           - 종합방송 비중(40% 이상)이 높은 상태에서 "시청층 관심사 다변화" 등으로 포장하거나 내용을 자의적으로 해석하지 마세요.
        ...
        1. 핵심 기류
        - (전일 대비 인물별 단독 조회수 역전/급등 및 당내/인물+전대 프레임 결합을 통한 투표 직전 여론 집결 양상을 2문장으로 요약)

        2. 주요 언급 키워드
        - (오늘 데이터 기반 주요 인물 및 신규 핵심 이슈 키워드 8~10개)

        3. 모니터링 관측 평가
        - (전일 대비 인물 관심도 역전 및 프레임 이동에 기반한 분석가 관점의 총평 1문장)
        """

        # ---------------------------------------------------------
        # 구글 API 공식 지원 표준 정규 모델명 적용
        # ---------------------------------------------------------
