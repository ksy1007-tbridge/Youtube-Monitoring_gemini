...
def generate_ai_insight(df, frame_stat_summary):
    ...
    try:
        ...
        # AI에게 전달 시 보정된 표 안의 데이터 수치만 엄격히 전달
        top_videos = df.head(10)[["채널명", "제목", "프레임", "조회수", "시간당조회수"]].to_dict(orient="records")

        prompt = f"""
        당신은 수치 변동과 구도 변화를 엄밀하게 추적하는 수석 데이터 분석가입니다.
        아래 제공된 [상위 영상 데이터]와 [프레임별 현황]만 '엄격히' 바탕으로 인사이트를 작성하세요.
        ...
        [엄격한 작성 및 분석 규칙]
        1. [단일 표본 과장 금지 및 총량 변화 반영]: '민생/경제/정책' 프레임이 단 1건인 경우, 개별 영상 효율은 높지만 총 조회수 비중(예: 전날 대비 하락 수치)의 한계를 함께 밝히세요. 단 1건의 수치로 시청자 관심이 전면 이동했다고 단정짓지 마세요.
        2. [실시간 공방 키워드 포착]: '친청계 스캔들', '반명몰이', '여론조사', '전대 역전' 등 진영 간 실시간으로 교체된 공격/대응 소재 및 프레임 변화를 '주요 언급 키워드'와 '핵심 기류'에 직접 반영하세요.
        3. [수치 팩트 엄수]: 데이터 표에 없는 시간당 조회수 수치(예: 45만 회 등)를 지어내거나 인용하지 마세요. 오직 제공된 수치만 인용하세요.
        ...
        1. 핵심 기류
        - (종합방송을 제외한 단일 프레임 현황 및 최근 진영 간 공방 소재 변화를 포함해 2문장으로 요약)

        2. 주요 언급 키워드
        - (단순 패널/출연자를 제외한 핵심 정치인 및 실시간 공방 키워드 나열)

        3. 모니터링 시사점
        - (단기 데이터 표본의 한계 및 프레임 변화 추이에 대한 분석가 관점의 제언 1문장)
        """
