...
def generate_ai_insight(df_top, frame_stat_summary, person_summary_str, trend_summary_str):
    ...
    try:
        ...
        prompt = f"""
        당신은 수치와 진영 간 프레임 전환을 정교하게 추적하는 수석 데이터 분석가입니다.
        ...
        [엄격한 작성 및 호칭 규칙 - 반드시 준수]
        1. [호칭 엄수]: 이재명은 현직 대한민국 대통령입니다. 지칭 시 반드시 '이재명 대통령'으로 엄격히 표기하세요.
        2. ['종합방송'/'기타' 단정 금지]: '종합방송'(판정 불가)이나 '기타' 프레임의 내부 논의 중심을 자의적으로 단정하여 서술하지 마세요.
        3. [호남 경선 공방 포착]: 호남 경선 임박에 따른 '노무현 계승/호남 표심' 관련 진영 간 정면충돌 공방(예: 노무현의 정청래 vs 정몽준의 김민석 / 정청래 호남 비하 논란 등)을 핵심 기류에 직접 반영하세요.
        4. [키워드 범주 제한]: '주요 언급 키워드'는 파편화된 단어를 제외하고 [주요 인물, 핵심 정치 이슈, 대립 사건]으로만 8~10개 엄선하여 작성하세요.
        ...
        1. 핵심 기류
        - (호남 경선 임박에 따른 노무현 계승/호남 표심 정면충돌 공방 및 전일 대비 변동 추세를 2문장으로 요약)

        2. 주요 언급 키워드
        - (주요 인물 및 핵심 정치 이슈 키워드 나열)

        3. 모니터링 관측 평가
        - (데이터 변동 및 프레임 전이 현상에 기반한 분석가 관점의 총평 1문장)
        """
